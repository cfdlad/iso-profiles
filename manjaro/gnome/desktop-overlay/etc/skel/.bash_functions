...
### Cheat
### Displays the cheat and previews the code 
### Press enter to paste the code
function ch() {
    cheat -l -c | awk '{print $1}' | fzf \
    --preview="cheat {} | bat -f -pp -l sh --theme='ansi'" \
    --preview-window=right,80% \
    --height 80% | \
      xargs -I{} cheat {} | bat -f -pp -l sh --theme='ansi'
	}
...
### Cheat-Path
### Displays only the cheat name and path (No code preview)
### Press enter to paste the code	
function ch-path() {
	cheat -l | fzf | awk '{print $1}' | \
	xargs -I{} cheat {} | bat -f -pp -l sh --theme='ansi'
	}
...
### Cheat-preview
### Displays only the code preview
### Press enter to paste cheat name and file path	
function ch-preview() {
	cheat -l | fzf \
	--preview="cheat -c {1}" \
	--preview-window=right,80% \
...
# use 'base16', 'OneHalfLight', 'Monokai Extended Light', 'ansi' for the theme
...
function pac-list() {
	...
	--preview "pacman -Qi {1} 2> /dev/null | bat -f -p -l yaml --theme='ansi'" \
	--prompt '  Find Packages > ' \
	| xargs -I{} pacman -Qi {} 2> /dev/null | bat -f -p -l yaml --theme='ansi' -P
	}
...
function pac-remove() {
	...
	--bind 'ctrl-t:transform:[[ ! {fzf:prompt} =~ List ]] &&
        ...
        echo "change-prompt(  Info Preview > )+change-preview:pacman -Qi \{} 2> /dev/null | bat -f -p -l yaml --theme='ansi'"' \
