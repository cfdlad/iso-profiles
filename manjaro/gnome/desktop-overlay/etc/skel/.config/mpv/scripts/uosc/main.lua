--[[ uosc | https://github.com/tomasklaen/uosc ]] --[[For playlist name bug goto line 931 and check line 942 for the fix]]
...
bind_command('playlist', create_self_updating_menu_opener({  		--Its here, the o2tvseries name/title bug
	title = t('Playlist'),											--It has been fixed in line 941 only
	type = 'playlist',												--To fix make 'serialize_path(item.filename).basename'
	list_prop = 'playlist',											--the first item after the '=' sign in line 941 
	...
	serializer = function(playlist)
		...
		for index, item in ipairs(playlist) do
			...
			items[index] = {
				title = serialize_path(item.filename).basename or (is_url and item.filename or item_title),
				hint = tostring(index),
				active = item.current,
				value = index,
			}
