...
function TopBar:init()
	...
	local max = {icon = 'crop_square', hover_bg = 'f7f7f7', command = maximized_command, is_max = true}
	local min = {icon = 'minimize', hover_bg = 'f7f7f7', command = function() mp.command('cycle window-minimized') end}
	...
	if options.top_bar_controls then
		...
		for _, button in ipairs(self.buttons) do
			if button.is_max then
				button.icon = state.fullscreen and 'filter_none' or
				(state.maximized and 'crop_square' or 'crop_square')
			end
